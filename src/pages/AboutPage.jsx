...
function AcademicItem({ item, index }) {
  ...
  return (
    <div
      ...
      className="relative flex flex-col items-center text-center px-3 py-5
                 cursor-default rounded-2xl transition-colors duration-300"
    ...
    >
      ...
      {/* Year */}
      <span className="text-[10px] font-mono mb-2 transition-colors duration-300"
...
function AcademicTimeline({ academicRefs, aRef }) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-2 relative mt-2">

      {/* Connecting line */}
      <div className="hidden md:block absolute top-[2.15rem] left-[12.5%] right-[12.5%] h-px"
           style={{
             background: "linear-gradient(to right, transparent, rgba(148,163,184,0.12), rgba(148,163,184,0.12), transparent)",
           }}
      />
      ...
      {ACADEMICS.map((item, i) => (
        <div
          ...
          style={{ opacity: 0 }}
        >
          <AcademicItem item={item} index={i} />
          {/* Mobile vertical connector */}
          {i < ACADEMICS.length - 1 && (
            <div className="md:hidden mx-auto w-px h-6 mt-1"
...
export default function AboutPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* Academic rows slide in */
      gsap.fromTo(academicRefs.current.filter(Boolean),
        { opacity: 0, x: 20 },
        {
          opacity: 1, x: 0,
        ...
        <div className="md:col-span-5 grid grid-cols-2 gap-4">
          {HIGHLIGHTS.map((h, i) => (
            <Card key={h.label} cardRef={bRef(i + 1)} className="p-5 flex flex-col justify-between">
              <span className="text-[10px] uppercase tracking-widest text-slate-600">{h.label}</span>
              <div className="mt-3">
